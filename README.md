...
```bash
docker run --rm -ti ghcr.io/woozymasta/archimate-ci:4.9.1-1.0.2 --help
```
...
pages:
  stage: build
  image:
    name: woozymasta/archimate-ci-image:4.9.1-1.0.2
    entrypoint: [""]
...
```bash
docker build \
  --tag archimate-ci:4.9.1-1.0.2 \
  --build-arg="ARCHI_VERSION=4.9.1-1.0.2" \
